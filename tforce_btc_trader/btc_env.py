...
class BitcoinEnv(Environment):
    # E: Environment comes from tensorforce library and it's just the skeleton for an environment, doesn't need to be changed
    EPISODE_LEN = 5000

    def __init__(self, hypers, cli_args={}):
        """Initialize hyperparameters (done here instead of __init__ since OpenAI-Gym controls instantiation)"""
        # E: the hypermarameters come from the bbdd where they are stored and updated in hypersearch
        ...
        # E: connects to the database that stores data and hyperparameters

        # gdax min order size = .01btc; kraken = .002btc
        self.min_trade = {Exchange.GDAX: .01, Exchange.KRAKEN: .002}[EXCHANGE]
        # E: exchanges have minimum trading volume and this takes this into account
        self.update_btc_price()
        # E: we need to change this and route it to our API: easy
        ...
        # E: same as before, this needs to connect to our database: also easy

        self.all_observations, self.all_prices = self.xform_data(all_data)
        # E: This function takes the raw data and preprocesses it as well as adds a few trading indices. It needs to be
        # heavily modified to adapt it to our needs. It returns "all observations", which are the indices,
        # and all_prices, which is just coinbase closing price (for BTC)

        self.all_prices_diff = self.diff(self.all_prices, True)
        # E: Further preprocessing by computing difference for closing price

        # Action space
        # E: this needs to be changed to invest in a different coin, we can also set three options but i would start
        # with a simple one and make sure it workds
    ...
    def diff(self, arr, percent=True):
        # E: computes percent change from a built in function in tforce (pct_change())
    ...
    def xform_data(self, df):
        """
        Some special handling of the price data. First, we don't want prices to be absolute, since we want the agent
        to learn actions _relative_ to states; that is, states need to be transformed into "relative" somehow. This
        ...
        # This needs to be adapted to our needs: that's a hard one. I am also worried about GPU usage in our case, where
        # data is much bigger

        # His data is in a dataframe with shape minutes x 8, where the 8 variables are (kaggle.py changes the names from
        # the database to):
        # timestamp, open, high, low, close, volume_btc, volume_currency, weighted_price

        # The hyperparameters and revenue indices are in another database named hyper_runs; which has the following
        # objects: id, hypers (hyperparameters), sharpes (revenue index
        # (https://www.investopedia.com/terms/s/sharperatio.asp)), returns (revenue gain), signals (??), prices,
        # uniques (unique parameters, for BO), flag (??) and agent (whether policy gradient or DQN). It creats on the
        # fly elements that can be found in hypersearch.py.

        columns = []
        ind_ct = self.hypers.indicators_count
        tables_ = data.get_tables(self.hypers.arbitrage) # E: arbitrage (another index) is set to False so it returns
                                                         # table[0], which is in data/data.py, line 96
        for table in tables_:
            for col in table['cols']:
                name_col = f'{table["name"]}_{col}'
                if name_col == data.target:     # E: data.target = 'coinbase_close', so, closing price. This needs to
                                                # be extended for every coin, not just BTC
                    columns.append(self.diff(df[name_col], True))
                elif col in table['price_cols']:
                    columns.append(df[name_col]/df[data.target])    # E: scale by closing price for open, high, low,
                                                                    # close, weighted_price columns: easy to implement
                else:
                    columns.append(df[name_col])

            # Add extra (trading) indicator columns
