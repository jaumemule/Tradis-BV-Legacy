...
dense_units = 128 * 1       # this number needs to be played with (last best = 128)
lstm_units = 64 * 2     # ídem (last best = 64)
dropout = 0.9           # also (last best = 0.2)
...
# This is the main run function
    ...
    X_train, y_train, scaler = class_functions.class_train(df, parameters)

    joblib.dump(scaler, 'class_scaler.save')
    ...
    model.save('BTC_20min_minutely_class.h5')
    ...
    if unix:
        file = open("logs/log_class" + str(name_time) + ".txt", "w")  # makeshift log -> needs to be improved
    else:
        file = open("logs\\log_class" + str(name_time) + ".txt", "w")  # makeshift log -> needs to be improved
    ...
    accuracy = []
    for i in range(round((df.shape[1] - training_number - seq_len - forecast_len) / forecast_len)):
        X_test, y_test = class_functions.class_test(df, seq_len, training_number + i * forecast_len, forecast_len, scaler)
