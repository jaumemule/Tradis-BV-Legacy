...
dense_units = 128 * 8      # this number needs to be played with (last best = 128)
lstm_units = 64 * 16     # ídem (last best = 64)
dropout = 0.1           # also (last best = 0.1)
learning_rate = 0.002   # (last best = 0.0001)
batch_size = 128        # try to optimize GPU capabilities
interval = 5
...
# This is the main run function
    ...
    pre_df = functions.data_load("data_20min.csv", range(6301))
    ...
    number = df.shape[1] - 24 * pred_days * 3 # We predict only the last n days

    parameters["number"] = number # add it to the dictionary

    print("Model trained on " + str(df.shape[0]/2) + " coins during " + str(round(number/3/24, 1)) + " days.")
    ...
    joblib.dump(scaler, 'scaler_20min.save')
    ...
    model.save('fast_model_20min.h5')  # we save the model

    print("Starting predictions over " + str(pred_days) + " days every " + str(forecast_len) + " periods of 20 minutes.")
    ...
    if unix:
        file = open("logs/log_20min" + str(name_time) + ".txt","w")  # makeshift log -> needs to be improved
    else:
        file = open("logs\\log_20min" + str(name_time) + ".txt","w")  # makeshift log -> needs to be improved
