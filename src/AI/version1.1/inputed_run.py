...
epochs = 10 # input the epochs from command line (for model testing purposes only)
...
seq_len = 240            # this is the amount of data (in minutes) used for the forecast
forecast_len = 60        # this is the forecast time in minutes
dense_units = 128 * 4       # this number needs to be played with (last best = 128)
lstm_units = 64 * 5     # ídem (last best = 64)
dropout = 0.2           # also (last best = 0.2)
learning_rate = 0.002   # (last best = 0.0001)
batch_size = 256*2        #
interval = 10            # explanation in following text:
...
# This is the main run function
    ...
    # 126017
    pre_df = functions.data_load_partial("dades_total.csv", 10000)
    ...
    aux_input = np.random.rand(df.shape[0])
    aux_input = np.tile(aux_input, (X_train.shape[0],1))
    ...
    elif which_model == "2reg_input":
        model = lstm.build_model2_reg_inputed(dims, dropout, learning_rate, aux_input)
    else:
    ...
    file.write("\nReturn amount is " + str(final_amount))
