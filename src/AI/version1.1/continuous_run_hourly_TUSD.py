...
class prediction_history(Callback):
    # This class is used to store the predictions (in form of predicted best coins)
...
# This is the main run function
    ...
    # We now convert to TUSD:

    df = df.iloc[::2,:]/df.loc["TUSD.p",:]
    ...
    #number = train_days * 24  # Choose this one or the one below
    number = df.shape[1] -  24 * pred_days # We predict only the last n days

    parameters["number"] = number # add it to the dictionary

    print("Model trained on " + str(df.shape[0]) + " coins during " + str(round(number/24, 1)) + " days.")
    ...
    joblib.dump(scaler, 'scaler_hourly_TUSD.save')
    ...
    model.save('model_hourly_TUSD.h5') # save the weights, just in case
